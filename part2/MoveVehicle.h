...
/// Dealing with the case of moving 0 cells
template<typename B, int R, int C>
struct MoveCarLeft<B, R, C, 0> {
    typedef  B updatedBoard;
};
...
struct MoveCarHorizontal<B, R, C, LEFT, A> {
    typedef typename MoveCarLeft<B, R, C, A>::updatedBoard updatedBoard;
};

template<typename B, int R, int C, int A>
struct MoveCarHorizontal<B, R, C, RIGHT, A>  {
    typedef typename MoveCarRight<B, R, C, A>::updatedBoard updatedBoard;
};
...
struct MoveCarVertical<B,R,C,UP,A> {
    ...
    typedef GameBoard<transposed_board> B_; /// Create a matching gameBoard
...
struct MoveVehicle<B, R, C, UP, A> {
    ...
    static_assert(GetCellDirectionInBoard<B, R, C>::D == DOWN || GetCellDirectionInBoard<B, R, C>::D == UP, 
    			"MoveVehicle: MOVE DIRECTION DOES NOT MATCH CAR ORIENTATION (must be parallel)"); /// Make sure the car moves in a legal direction
	typedef typename MoveCarVertical<B, R, C, UP, A>::updatedBoard board;
};
...
struct MoveVehicle<B, R, C, DOWN, A> {
    ...
    static_assert(GetCellDirectionInBoard<B, R, C>::D == DOWN || GetCellDirectionInBoard<B, R, C>::D == UP,
    			"MoveVehicle: MOVE DIRECTION DOES NOT MATCH CAR ORIENTATION (must be parallel)"); /// Make sure the car moves in a legal direction
    typedef typename MoveCarVertical<B, R, C, DOWN, A>::updatedBoard board;
};
...
struct MoveVehicle<B, R, C, LEFT, A> {
    ...
    static_assert(C >= 0 && C < B::width, "MoveVehicle: COLUMN NUMBER IS ILLEGAL (must be between 0 and the length of the board)");
    static_assert(GetCellTypeInBoard<B, R, C>::T != EMPTY, "MoveVehicle: CANNOT MOVE AN EMPTY CELL"); /// If it's EMPTY, fail the compilation
    static_assert(GetCellDirectionInBoard<B, R, C>::D == LEFT || GetCellDirectionInBoard<B, R, C>::D == RIGHT,
    			"MoveVehicle: MOVE DIRECTION DOES NOT MATCH CAR ORIENTATION (must be parallel)"); /// Make sure the car moves in a legal direction
    typedef typename MoveCarHorizontal<B, R, C, LEFT, A>::updatedBoard board;
};
...
struct MoveVehicle<B, R, C, RIGHT, A> {
    ...
    static_assert(GetCellDirectionInBoard<B, R, C>::D == LEFT || GetCellDirectionInBoard<B, R, C>::D == RIGHT,
    			"MoveVehicle: MOVE DIRECTION DOES NOT MATCH CAR ORIENTATION (must be parallel)"); /// Make sure the car moves in a legal direction
    typedef typename MoveCarHorizontal<B, R, C, RIGHT, A>::updatedBoard board;
};
